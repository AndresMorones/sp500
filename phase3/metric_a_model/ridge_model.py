"""Metric A Model — Ridge regression.
...
from config import METRIC_A_TICKERS, TARGETS, RIDGE_ALPHAS, LOOKBACK, RESULTS_DIR
...
def run_ridge(ticker, target, series, cat_cols):
    """Train Ridge: news features → A score, convert to price."""
    ...
    X_val = scaler.transform(X[train_end:val_end]) if val_end > train_end else np.array([]).reshape(0, X.shape[1])
    X_test = scaler.transform(X[val_end:])

    y_train = y_a[:train_end]
    y_test_a = y_a[val_end:]
    test_dates = dates[val_end:]
    test_prev_closes = prev_closes[val_end:]
    test_has_news = has_news[val_end:]

    # Train Ridge on A prediction (train + val)
    X_fit = np.vstack([X_train, X_val]) if len(X_val) > 0 else X_train
    y_fit = np.concatenate([y_train, y_a[train_end:val_end]]) if val_end > train_end else y_train
    ...
    # Convert predicted A → price (baseline: avg S&P return)
    pred_prices, pred_returns = a_to_price(pred_a, t_alphas, t_betas, t_s0s, sp_avg, test_prev_closes)
    ...
    # Feature importance (top 10 by |coef|)
    coef_abs = np.abs(model.coef_)
    top_idx = np.argsort(-coef_abs)[:10]
    top_feats = [(columns[i], float(model.coef_[i])) for i in top_idx]
...
def main():
    print("=" * 60)
    print("  Metric A Model — Ridge (news → A → price)")
    print("=" * 60)
    ...
    for target in TARGETS:
        ...
        for ticker in METRIC_A_TICKERS:
            ...
            result = run_ridge(ticker, target, series, cat_cols)
            if result is None:
            ...
            print(f"  Price: MAE=${result['MAE']:.3f}  MAPE={result['MAPE']:.3f}%  Alpha={result['alpha']:.2f}")
            ...
            print(f"\n  Top features (news categories → A):")
            for feat, coef in result["top_features"][:5]:
                print(f"    {feat:<40} {coef:>+8.6f}")

    # Save results
    out_path = os.path.join(RESULTS_DIR, "metric_a_ridge_results.csv")
    ...
    pred_path = os.path.join(RESULTS_DIR, "metric_a_ridge_predictions.csv")
