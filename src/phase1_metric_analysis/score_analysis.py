...
def build_rankings(rows, n):
    """Build top-N / bottom-N rankings for all metrics and periods."""
    rankings = {}
    for m in METRICS:
        for p in PERIODS:
            col = f"{m}_{p}"
...
def build_forward_index(rows):
    """Build per-ticker timeline and date index for forward return lookups."""
    ticker_timeline = {}
    ticker_date_idx = {}
    for r in rows:
        t = r["ticker"]
    ...
    return ticker_timeline, ticker_date_idx


def forward_returns(row, horizon, ticker_timeline, ticker_date_idx):
...
def compute_forward_stats(event_rows, ticker_timeline, ticker_date_idx, max_horizon=5):
    ...
    for row, score in event_rows:
        fwd = forward_returns(row, max_horizon, ticker_timeline, ticker_date_idx)
        if fwd is None:
            continue
        cum_stock = 1.0
        cum_sp = 1.0
...
def continuation_rate(event_rows, ticker_timeline, ticker_date_idx, max_horizon=5):
...
def main():
    rows = load_data()
    rankings = build_rankings(rows, N)
    ticker_timeline, ticker_date_idx = build_forward_index(rows)
    ...
    for fk in FORWARD_K_VALUES:
        ...
        for p in PERIODS:
            for direction, label in [("high", f"TOP {fk} (positive scores)"), ("low", f"BOTTOM {fk} (negative scores)")]:
                ...
                for m in METRICS:
                    stats = compute_forward_stats(rk[(m, p, direction)], ticker_timeline, ticker_date_idx)
                ...
                # Individual daily stock returns (raw)
        ...
        for p in PERIODS:
            for direction, label in [("high", f"TOP {fk}"), ("low", f"BOTTOM {fk}")]:
                ...
                for m in METRICS:
                    rates = continuation_rate(rk[(m, p, direction)], ticker_timeline, ticker_date_idx)
                    line = f"{m:<8}"
                    for h in HORIZONS:
                        line += f" {rates[h]:>9.1f}%"
                    print(line)
    ...
    # Cache rankings per K to avoid redundant recomputation
    rankings_cache = {fk: build_rankings(rows, fk) for fk in FORWARD_K_VALUES}
    ...
    for m in METRICS:
        ...
        for fk in FORWARD_K_VALUES:
            ...
            for p in PERIODS:
                for direction in ["high", "low"]:
                    r = continuation_rate(rk[(m, p, direction)], ticker_timeline, ticker_date_idx)
                    rates.append(r[1])
