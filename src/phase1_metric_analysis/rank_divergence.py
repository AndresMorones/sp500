    ...
    for ticker in tickers:
        ...
        for period in ["gap", "cc"]:
            for direction in ["high", "low"]:
                ...
                print(f"\n{'=' * 130}")
                print(f"  {ticker} -- {label} -- Rank differences between {', '.join(METRICS)}")
                print(f"{'=' * 130}")
                ...
                # Find all unique events across all metrics' top N
                all_keys = set()
                for m in METRICS:
                    for r in rankings[m]:
                ...
                if not event_data:
                    print(f"  No differences -- all {len(METRICS)} metrics agree on the same top {n}.")
                    continue
