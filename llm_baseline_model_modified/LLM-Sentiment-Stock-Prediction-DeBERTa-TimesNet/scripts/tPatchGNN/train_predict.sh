...
for seed in 16 32 64 128 256 512 1024 2048 4096 8192; do
    for model in "deberta" "finbert" "roberta"; do
        for target_column in "Float_Price" "Factor_Price" "Delta_Price" "Binary_Price"; do
            ...
            for ticker in AAPL AMZN GOOGL META MSFT NVDA TSLA; do
                echo "Training tPatchGNN for ${ticker} with ${model} model, ${target_column} target, seed ${seed}"
                ...
                python stock_prediction/modeling/train_tpatchgnn.py \
                    ...
                    --scaler_path models/${ticker}_tpatchgnn_${target_column}_${model}_scaler.pkl \
